...
from utils.default import groupExtra
...
class Jishaku(commands.Cog):  # pylint: disable=too-many-public-methods
    ...
    @groupExtra(name="jishaku",
                    usage="jishaku [subcommand] ...",
                    brief="The Jishaku debug and diagnostic commands",
                    aliases=["jsk"],
                    hidden=True,
    ...
    async def jsk(self, ctx: commands.Context):
        ...
        """
        desc=""
        jsk_em=discord.Embed(color=self.bot.embed_color)
        jsk_em.title="JSK Info"
#        summary = [
...
#        ]
        ...
        desc+=f"**Jishaku version:** `v{__version__}`\n"
        desc+=f"**Discord.py version:** `{discord.__version__}`\n"
        desc+=f"**Python version:** `{sys.version}`\n\n"
        desc+=f"**Running on:** `{sys.platform}`\n"
        desc+=f"Module was loaded {humanize.naturaltime(self.load_time)}, cog was loaded {humanize.naturaltime(self.start_time)}\n\n"
...
#                summary.append(f"Using {humanize.naturalsize(mem.rss)} physical memory and "
                ...
                jsk_em.add_field(name="Memory Info:", value=f"Using {humanize.naturalsize(mem.rss)} physical memory and {humanize.naturalsize(mem.vms)} virtual memory, {humanize.naturalsize(mem.uss)} of which unique to this process.")

                name = proc.name()
                pid = proc.pid
                thread_count = proc.num_threads()

                desc+=f"Running on PID {pid} (`{name}`) with {thread_count} thread(s)."
...
#                summary.append("")  # blank line
        ...
        if isinstance(self.bot, discord.AutoShardedClient):
            jsk_em.add_field(name="Bot Info:", value=f"This bot is automatically sharded and can see {cache_summary}.\n\nAverage websocket latency: {round(self.bot.latency * 1000, 2)}ms")
#            summary.append(f"This bot is automatically sharded and can see {cache_summary}.")
        elif self.bot.shard_count:
            jsk_em.add_field(name="Bot Info:", value=f"This bot is manually sharded and can see {cache_summary}.\n\nAverage websocket latency: {round(self.bot.latency * 1000, 2)}ms")
#            summary.append(f"This bot is manually sharded and can see {cache_summary}.")
        else:
            jsk_em.add_field(name="Bot Info:", value=f"This bot is not sharded and can see {cache_summary}.\n\nAverage websocket latency: {round(self.bot.latency * 1000, 2)}ms")
#            summary.append(f"This bot is not sharded and can see {cache_summary}.")

#        summary.append(f"Average websocket latency: {round(self.bot.latency * 1000, 2)}ms")

#        await ctx.send("\n".join(summary))
        jsk_em.description=desc
        jsk_em.set_thumbnail(url="https://cdn.discordapp.com/attachments/427826132260487168/562303750589513743/PicsArt_04-01-05.54.17.png")

        await ctx.send(embed=jsk_em)
    ...
    @jsk.command(name="shutdown", aliases=["logout", "kys", "reboot"])
